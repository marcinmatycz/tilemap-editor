...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
            ...
            (highlighted_texture_tile.has_value()))
        {
            ...
            if ((tile.x < 0.0f) or (tile.y < 0.0f) or ((tile.x + tile.width / 2) > tex.width) or
                ((tile.y + tile.height / 2) > tex.height))
            {
        ...
        if (app_state.selected_tile.has_value())
        {
            ...
            const Rectangle tile_following_mouse{.x = inputs.mouse_point.x,
                                                 .y = inputs.mouse_point.y,
                                                 .width = app_state.main_grid.square_size_px,
                                                 .height = app_state.main_grid.square_size_px};
            DrawTexturePro(app_state.tilemaps[app_state.tilemap_index].texture, app_state.selected_tile.value(),
                           tile_following_mouse, {}, 0.f, WHITE);
            DrawRectangleLines(tile_following_mouse.x, tile_following_mouse.y, tile_following_mouse.width,
                               tile_following_mouse.height, BLACK);
        }
